...
1. **Build a lightweight seed check, not the FPD stability layer.** Run the
   per-user split under ~3 to 5 seeds and report the **mean and std** of NDCG@10 /
...
3. **Cold-start fallback is for items/edge users, sized small on the user side.**
   Only ~8 to 21 users are thin; the fallback's main value is a sensible default
...
## Postscript, the prediction held (Milestones 3 to 4)
...
deviation ranged ~0.0003 to 0.0031 against means of 0.12 to 0.17, i.e. seed noise is
~0.2 to 2% of signal, and config differences are far larger than the bands. The
